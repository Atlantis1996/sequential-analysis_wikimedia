...
public final class DataFilter {
    ...
    static boolean checkDomain(final String[] columns) {
        // throw new RuntimeException("To be implemented");
        return columns[0].equals("en") || columns[0].equals("en.m");
    }
    ...
    static boolean checkSpecialPage(final String[] columns) {
        // throw new RuntimeException("To be implemented");
        return !(columns[1].equals("404.php") || columns[1].equals("Main_Page") ||  columns[1].equals("-"));
    }

    /**
     * Check if the title starts with any blacklisted prefix, case insensitive.
     *
     * <p>Any occurrences of {@code %3a} should have been decoded into {@code :}.
     *
     * @param columns record as columns
    ...
    static boolean checkPrefix(final String[] columns) {
        // throw new RuntimeException("To be implemented");
        for (String str : PREFIX_BLACKLIST) {
            if (StringUtils.startsWith(columns[1].toLowerCase(), str)) {
                return false;
            }
        }
        return true;
    ...
    static boolean checkSuffix(final String[] columns) {
        // throw new RuntimeException("To be implemented");
        for (String str : SUFFIX_BLACKLIST) {
            if (StringUtils.endsWith(columns[1].toLowerCase(), str)) {
                return false;
            }
        }
        return true;
